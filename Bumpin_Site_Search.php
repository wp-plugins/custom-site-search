<?php
/*
	Plugin Name: BumpIn Site Search
...
function render_bumpin_site_search( $opt, $before_title, $after_title) {
...
function widget_bumpin_site_search_control( $args=null ) {
...
?>
		<p style="text-align:justify;">It seems you are already using BumpIn SocialBar widgets. </p>
                <p style="text-align:justify; color:red;">If you dont see the "Site Search" app in the SocialBar at your site, Please follow the steps below.</p>
...
function widget_bumpin_site_search($args) {
	extract($args);
	$options = get_option('BumpIn');
	echo $before_widget;
	render_bumpin_site_search($options,$before_title,$after_title);
  	echo $after_widget;
}

function bumpin_site_search_init()
{
	register_sidebar_widget(__('Custom Site Search'), 'widget_bumpin_site_search');
	register_widget_control('Custom Site Search','widget_bumpin_site_search_control',600,600);    /* width, height*/
}
add_action("plugins_loaded", "bumpin_site_search_init");
?>
